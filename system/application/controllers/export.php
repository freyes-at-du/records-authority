 ...
 class Export extends Controller {
 	...
 	public function transform() {
 		...
 		if ($this->uri->segment(3)) {
			...
			if ($format == "pdf") {
    			...
    			$this->toPdf($html, 'retention_schedule');
			}
			
			if ($format == "csv") {
				$this->toCsv($getRetentionScheduleQuery);
			}
	...
	private function getRetentionScheduleIDs($departmentID) {
		...
		if($departmentID != 999999) {
	 	...
	 	$this->db->select('retentionScheduleID, recordName, recordCategory, recordDescription, retentionPeriod, primaryAuthorityRetention, retentionDecisions, primaryAuthority, notes, vitalRecord, approvedByCounsel, approvedByCounselDate, officeOfPrimaryResponsibility, disposition');
	...
	private function toPublic($headers,$line) {
		$data = "retention_schedules";
		$this->load->view('includes/adminHeader',$data);
		$filename = "retention_schedules";

...
/**
	...
	private function generateSolrResults($keyword, $result, $sortBy, $recordCount) {
		...
		$retentionScheduleResults .= "<th width='18%'><strong>Description</strong></th>";
		$retentionScheduleResults .= "<th width='12%'><strong><a href='#' title='Click to sort' onClick='sortBy($kw);'>Search Terms</a></strong></th>";
		...
		foreach ($result['response'] as $response) {
			if (is_array($response)) {
				foreach ($response as $docs) {
					...
					$retentionScheduleResults .= "<td width='12%'>";

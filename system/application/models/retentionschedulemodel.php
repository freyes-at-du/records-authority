...
class RetentionScheduleModel extends Model {
	...
	private function getRetentionScheduleRecordQuery($retentionScheduleID) {
		$sql = "SELECT retentionScheduleID, recordName, recordCategory, recordDescription, retentionPeriod, disposition, officeOfPrimaryResponsibility, approvedByCounselDate FROM rm_fullTextSearch " .
	...
	private function indexRetentionSchedules($retentionScheduleID) {
		...
		$this->db->select('retentionScheduleID, recordName, recordDescription, recordCategory, retentionPeriod, disposition, officeOfPrimaryResponsibility, approvedByCounselDate');
		$this->db->from('rm_retentionSchedule');

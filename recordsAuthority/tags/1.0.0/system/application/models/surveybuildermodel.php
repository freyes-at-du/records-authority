    ...
    */
 	private function addSurveyNameQuery($surveyName) {
		...
		if ($query->num_rows() > 0) {
			...
			send_email($this->devEmail, 'Liaison_Error', 'database error: Survey name not returned. addSurveyNameQuery()');
 	...
 	private function addSurveyQuestionQuery($surveyQuestion) {
 		...
 		if (element('subQuestion', $surveyQuestion) == 1) {
			...
			if ($query->num_rows() > 0) {
				...
				send_email($this->devEmail, 'Liaison_Error', 'database error: question ID not returned. addSurveyQuestionQuery()');
 	...
 	private function addSurveySubQuestionQuery($surveySubQuestion, $surveySubChoiceQuesiton) {
 		...
 		if (element('subChoiceQuestionCheck', $surveySubQuestion) == 1) {
			...
			if ($query->num_rows() > 0) {
				...
				send_email($this->devEmail, 'Liaison_Error', 'database error: unable to retrieve subQuestionID addSurveySubQuestionQuery()');
